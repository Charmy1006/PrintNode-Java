...
public class APIClient {
	 ...
	 *
	 * @param url
	 *            New url to be set for use with the library.
	 */
	 ...
	 *
	 * @param creatorRef
	 *            creator refence of child account.
	 */
	 ...
	 * @param enabled
	 *            whether you are disabling or enabling the clients.
	 ...
	 *
	 * @param printjobinfo
	 *            PrintJobJson object with values set.
	 ...
	 *
	 * @param computerId
	 *            the computer id we want to find scales for
	...
	public final Printer[] getPrinterforChildAccount() throws IOException {
		...
		Printer[] printers;
		try {
			HttpGet httpget = new HttpGet(apiUrl + "/printers/");
			httpget.addHeader(childHeaders[0], childHeaders[1]);
			CloseableHttpResponse response = client.execute(httpget);
			try {
				JsonArray responseParse = responseToJsonElement(response).getAsJsonArray();
	...
	public final Printer[] getPrinterCapabilities(int id) throws IOException {
		...
		Printer[] printers;
		try {
			HttpGet httpget = new HttpGet(apiUrl + "/printers/" + id);
			httpget.addHeader(childHeaders[0], childHeaders[1]);
			CloseableHttpResponse response = client.execute(httpget);
			try {
	 ...
	 *            the UUID.
	 * @param edition
	 *            The edition of the client the client-key refers to.
	 * @param version
	 *            the version of client being used.
	 * @return a client key for the speicfied arguments.
	 * @throws IOException
	 *             if HTTP client is given bad values
	 */
	public final String getClientKey(final String uuid, final String edition, final String version) throws IOException {
		CloseableHttpClient client = HttpClients.custom().setDefaultCredentialsProvider(credentials).build();
